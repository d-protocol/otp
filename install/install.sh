usage() { echo "Usage: $0 [-v <version number>] [-r <docker repo>] [-m] [-l] where -v is the version number, -r is the docker repo, -m for microk8s, -l for local build" 1>&2; exit 1; }

DOCKERREPO="ettec"

while getopts ":v:r:m:l" o; do
    case "${o}" in
        v)
            VERSION=${OPTARG}
            ;;
...
if [ "$LOCAL_BUILD" = "true" ]; then
    build_local_images
    if [ "$USEMICROK8S" = "true" ]; then
        load_images_to_microk8s
    fi
    DOCKERREPO="localhost"
fi

if [ "$LOCAL_BUILD" = "true" ]; then
    helm install --wait --timeout 1200s otp-${VERSION} ../helm-otp-chart/ -f local-values.yaml
else
